...
export type FetchPackageStatsResult = {
  name: string;
...
export type FetchPackageStatsOptions = {
  group: 'major' | 'minor' | 'patch';
};

export async function fetchPackageStats(
  packageName: string,
  options: FetchPackageStatsOptions,
): Promise<FetchPackageStatsResult> {
...
export function printPackageStats(pkg: FetchPackageStatsResult, options: CliOptions) {
  const statsToDisplay = filterStats(pkg.stats, {
  ...
  console.log(chalk.bold(`\nNPM weekly downloads for ${primaryColor(pkg.name)}\n`));
  console.log(`Total: ${primaryColor(pkg.totalDownloads.toLocaleString())} last week\n`);

  console.log(
    options.top
      ? `Top ${options.top} ${pkg.groupingType} versions:\n`
      : `By ${pkg.groupingType} version:\n`,
  );

  const maxDownloads = Math.max(...pkg.stats.map((v) => v.downloads));
  ...
  console.log(chalk.bold(`\nNPM weekly downloads for ${chalk.green(pkg.name)}\n`));
  console.log(`Total: ${chalk.green(pkg.totalDownloads.toLocaleString())} last week\n`);
}
